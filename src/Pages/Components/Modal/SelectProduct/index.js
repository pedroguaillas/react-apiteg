...
class SelectProduct extends Component {
    ...
    componentDidMount() {
        ...
        let items = (products.length !== 0 && id > 0) ? products.filter(product => product.id === id) : null
    ...
    componentDidUpdate(prevProps) {
        ...
        if (this.props.id !== prevProps.id && this.props.id > 0) {
            ...
            if (this.props.products !== prevProps.products && this.props.products.length > 0) {
                this.setState((state, props) => ({
                    item: props.products.filter(product => product.id === props.id)[0].atts.name
                }))
            } else {
    ...
    toggle = async () => {
        ...
        if (products.length === 0) {
            ...
            try {
                // await clientAxios.post('productlist', { paginate: 10 })
                await api.post('productlist', { paginate: 10 })
                    .then(res => {
    ...
    reqNewPage = async (e, page) => {
        ...
        if (page !== null) {
            ...
            try {
                // await clientAxios.post(`productlist?page=${page.substring((page.indexOf('=')) + 1)}`, { search, paginate: 10 })
                await api.post(`productlist?page=${page.substring((page.indexOf('=')) + 1)}`, { search, paginate: 10 })
                    .then(res => {
                        let { data, links, meta } = res.data
                        this.setState({
                            products: data,
    ...
    onChangeItem = async (e) => {
        ...
        if (value.length > 1) {
            try {
                // await clientAxios.post('productlist', { search: value, paginate: 5 })
                await api.post('productlist', { search: value, paginate: 5 })
                    .then(res => {
                        let { data, links, meta } = res.data
                        this.setState({
                            item: value,
    ...
    onChangeSearch = async (e) => {
        ...
        try {
            // await clientAxios.post('productlist', { search: value, paginate: 10 })
            await api.post('productlist', { search: value, paginate: 10 })
                .then(res => {
                    let { data, links, meta } = res.data
                    this.setState({
                        search: value,
                            ...
                            <ListGroup style={{ 'cursor': 'pointer' }}>
                                {
                                    suggestions.map((suggestion, index) => (
                                        <ListGroupItem onClick={() => this.props.selectProduct(suggestion, this.props.index)} style={{ padding: '.4em', 'font-size': '.9em', 'text-align': 'left' }} key={index}>
                                    ...
                                    {products.map((product, index) => (
                                        <tr key={index}>
                                            ...
                                            <td onClick={() => this.selectProduct(product)}>
                                                <a href="javascript:void(0);" className="alert-link">{product.atts.name}</a>
                                            </td>
                                        </tr>
                                    ))}
                                </tbody>
