...
class SelectCustomer extends Component {
  ...
  //Show & hidden modal
  toggleNewCustomer = () =>
    this.setState(state => ({ modalnewcust: !state.modalnewcust }))
  ...
  submit = async () => {
    if (this.validate()) {
      try {
        await api.post('customers', this.state.customer).then(res => {
          let { customer } = res.data
          let { customers } = this.state
          customers.push({
            id: customer.id,
            atts: { identication: customer.identication, name: customer.name }
          })
  ...
  validate = () => {
    ...
    if (
      customer.type_identification === 'cédula' &&
      customer.identication.trim().length !== 10
    ) {
    ...
    if (
      customer.type_identification === 'ruc' &&
      customer.identication.trim().length < 13
    ) {
    ...
    if (
      customer.type_identification === 'pasaporte' &&
      customer.identication.trim().length < 3
    ) {
  ...
  toggle = async () => {
    ...
    if (customers.length === 0) {
      try {
        await api.post('customerlist', { paginate: 10 }).then(res => {
  ...
  reqNewPage = async (e, page) => {
    ...
    if (page !== null) {
      ...
      try {
        await api
          .post(`customerlist?page=${page.substring(page.indexOf('=') + 1)}`, {
            search,
            paginate: 10
          })
          .then(res => {
    ...
    if (value.length > 1) {
      ...
      if (!this.state.searching) {
        try {
          ...
          await api
            .post('customerlist', { search: value, paginate: 4 })
            .then(res => {
              let { data, links, meta } = res.data
              this.setState({
                // item: value,
  ...
  onChangeSearch = async e => {
    ...
    try {
      await api
        .post('customerlist', { search: value, paginate: 10 })
        .then(res => {
          let { data, links, meta } = res.data
          this.setState({
            search: value,
    ...
    return (
      <Fragment>
            ...
            <Col style={{ width: '100%' }}>
              <ListGroup style={{ cursor: 'pointer' }}>
                {suggestions.map(({ id, atts }, index) => (
                  <ListGroupItem
                    onClick={() => this.props.selectCustomer(id)}
                    style={{
                      padding: '.4em',
                      'font-size': '.9em',
                      'text-align': 'left'
        ...
        <Modal
          ...
          centered={true}
        >
          <ModalHeader toggle={this.toggleNewCustomer}>
            Registrar cliente
          </ModalHeader>
          <ModalBody>
