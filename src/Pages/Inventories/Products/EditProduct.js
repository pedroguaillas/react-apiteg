...
class EditProduct extends Component {
    ...
    async componentDidMount() {
        ...
        try {
            await clienteAxios.get('products/' + params.id)
                .then(res => {
                    this.setState({
                        form: res.data.product,
                        unities: res.data.unities,
                        categories: res.data.categories,
                        accounts: res.data.accounts,
                        entry: res.data.product.entry_account_id !== null,
                        active: res.data.product.active_account_id !== null,
                        inventory: res.data.product.inventory_account_id !== null
                    })
                });
        } catch (error) {
            console.log(error)
    ...
    submit = async () => {
        ...
        tokenAuth(this.props.token);
        try {
            await clienteAxios.put('products/' + params.id, this.state.form)
    ...
    render() {
        ...
        return (
            <Fragment>
                <PageTitle
                    options={[
                        { type: 'button', id: 'tooltip-save-product', action: this.submit, icon: 'save', msmTooltip: 'Guardar producto', color: 'primary' },
                    ]}
                    heading="Producto"
                                ...
                                <CardBody>
                                    <Form className="text-right">
                                        ...
                                        <Row form>
                                            ...
                                            <Col sm={3}>
                                                <FormGroup className="mb-1" row hidden={!(this.state.inventory)}>
